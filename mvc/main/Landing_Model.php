  ...
  class Landing_Model extends Admin_Model{
	...
	public function Apply_Feform_Application_Page($ApplyCode='',$ShowType='preview'){
	  ...
	  try{ 	  
		...
		$license[3] .= "  <footer>林務局自然保留區暨自然保護區進入申請系統 敬啟</footer> ";
	...
	public function Landing_Regist_Mail($DataCode=0 ){
	  ...
	  try{  
		...
		$mail_content .= "<div> </div>";
		$mail_content .= "<div>四、為維護自然生態，各區設置承載量管制，若申請截止日(依據各區設定)總人數逾越承載量，系統將進行隨機抽籤，若該區未逾越承載量，則無須抽籤，自動將資格設為正取，並發給「正取送審」、「備取送審」、「抽籤未中」等狀態之電子郵件通知。</div>";
		$mail_content .= "<div> </div>";
